...
$core_version = 11; /* change core version when update */
...
add_action('init', function() {
						...
						add_menu_page(						
							...
							function() use ($am){
								...
								<div id="cloodo_admin_default" style="display:block;">
									<iframe id="cloodo_iframe" src="<?php echo esc_url($am['target_url']) ?>" frameborder="0" style="width: 100%; height: 100vh;"></iframe>
								</div>	
								<?php
								}
								/* include view */
	...
	if (isset($_GET['token'])) {
		...
		$companies_res = json_decode(wp_remote_get(CLOODO_BASE_API . 'companies', $author_token)['body']);
		if (isset($response->message) && ($response->message == 'Retrieved successfully' )){
			update_option('cloodo_token', $token);
			update_option('cloodo_data', $response->data);
			update_option('cloodo_company', $companies_res->data);
